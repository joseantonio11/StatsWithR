...
smoke1 <- subdata %>% dplyr::filter(smoke100 %in% c("Yes","No"))
## creating the plot
...
subdata1 <- brfss2013[vars1]
alcohol1 <- subdata1 %>% dplyr::filter(!is.na(alcday5))
## plotting
...
```{r, echo=TRUE}
alcohol2 <- subdata1 %>% dplyr::filter(!is.na(avedrnk2))
...
```{r, echo=TRUE}
alcohol3 <- subdata1 %>% dplyr::filter(!is.na(drnk3ge5))
...
subdat4 <- brfss2013[vars4]
subdat4 <- subdat4 %>% dplyr::filter(!is.na(physhlth))
g <- ggplot(subdat4, aes(x = as.factor(genhlth))) + geom_bar()
g
```
...
subdat5 <- brfss2013[vars5]
subdat5 <- subdat5 %>% dplyr::filter(!is.na(menthlth))
g <- ggplot(subdat5, aes(x = as.factor(genhlth))) + geom_bar()
g
```
...
subdat6 <- brfss2013[vars6]
subdat6 <- subdat6 %>% dplyr::filter(!is.na(poorhlth))
g <- ggplot(subdat6, aes(x = as.factor(genhlth))) + geom_bar()
g
```
The previous graphic show that there is a lot of people with stress, depression and problems emotions.The majority enjoy Excellent, Very Good and Good Health (with small health problems).
